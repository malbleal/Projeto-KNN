...
def dist_euclidiana(no_class, data): # Função que calcula a distancia euclidiana entre 'no_class[i]' e todos os elementos de data.
    dist_eucl = []

    for i in range(len(no_class)):
        aux = []
        ponto1 = no_class[i][2]  # Coordenadas de no_class

        for item in data:
            ponto2 = item[2]  # Coordenadas de data
            soma = np.sqrt((ponto1[0] - ponto2[0])**2
                  + (ponto1[1] - ponto2[1])**2
                  + (ponto1[2] - ponto2[2])**2
                  + (ponto1[3] - ponto2[3])**2
                  )
            aux.append([soma, item[1]])

        dist_eucl.append(aux)

    return dist_eucl

def tipagem(resultado, j, k): # É necessário o tratamento dos dados pela função 'dist_euclidiana' antes de chamar essa função.
    aux = []

# Esse 'for' puxa todos elementos de 'resultado' para dentro de uma lista auxiliar.
    for x in range(len(data)):
        aux.append(resultado[j][x][0])
    menores_dist = sorted(aux)[:k] # organiza, por ordem crescente, as 120 distancias de dentro 
                                   # de cada elemento 'aux' e retira 'k' elementos.

# Esse 'for' transforma 'menores_dist' na classificação. Igualando os 3 menores pegos pelo for anterior a lista de 120 elementos.
    for i in range(len(menores_dist)):
        for x in range(len(data)):
            if np.array_equal(menores_dist[i], resultado[j][x][0]):
                menores_dist[i] = resultado[j][x][1]

# Essa parte do código conta qual a recorrencia de cada status dos vizinhos do investidor.
    cont_c = menores_dist.count('Conservador')
    cont_m = menores_dist.count('Moderado')
    cont_a = menores_dist.count('Agressivo')
    maior = cont_c

# e essa parte compara qual é o maior e retorna o 'tipo'.
    if cont_m > maior:
        return 'Moderado'
    if cont_a > maior:
        return 'Agressivo'
...
def classificar(tipo, i): # É necessário o tratamento dos dados pela função 'tipagem' antes de chamar essa função.
        no_class[i][1] = tipo # Classifica o tipo do elemento 'no_class' de acordo com o retorno da função 'tipagem'. 
...
# Chamamos a função 'dist_euclidiana' e a guardamos o resultado em 'dist_eucl'
dist_eucl = dist_euclidiana(no_class, data)

# Nesse 'for' percorremos a lista 'dist_eucl' pelo seu tamanho.
for i in range(len(dist_eucl)):
    var = tipagem(dist_eucl, i, 3) # Tipando cada linha. Para cada 'i', 'var' recebe um tipo,
    classificar(var, i)         # e classifica do primeiro ao ultimo.


# DataFrame para melhor visualizar os dados do programa.
